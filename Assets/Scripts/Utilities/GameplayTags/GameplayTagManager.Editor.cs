...
namespace Assets.Scripts.Data.Tags
{
    public partial class GameplayTagManager
    {
        ...
        private readonly List<GameplayTagInfo> _tagList = new List<GameplayTagInfo>();
        ...
        private void UpdateGameplayTagInfo()
        {
            ...
            foreach (GameplayTag tag in _gameplayTags)
            {
                ...
                for (int j = 0; j < tagPath.Length; ++j)
                {
                    ...
                    if (index == -1)
                    {
                        GameplayTagInfo info = new GameplayTagInfo {Name = tagName, Level = j, ParentIndex = parentIndex, ChildIndex = new List<int>()};
                        if (parentIndex == -1)
                        {
